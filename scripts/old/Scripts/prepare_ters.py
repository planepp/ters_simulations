...
if not np.allclose(com_unconstrained[:2], [0, 0], atol=1e-6):
    print(f"WARNING: molecule COM may not at origin! Offset = {com_unconstrained[:2]}")

    atoms = read('geometry.in')
    atoms.translate([-com_unconstrained[0], -com_unconstrained[1], 0])
