...
def read_gaussian(file_name):
    ...
    def read_frequencies_red_masses(N1, N2, file):
        ...
        if N1 == 2 and N2 == 0:
            frequencies, current_line, current_element = read_n_elements(3*N1-5, 0, line_data, file)
            red_masses, _, _ = read_n_elements(3*N1-5, current_element, current_line, file)
        elif N2 == 0:
    ...
    def read_atom_an(N1, N2, file):
        ...
        line_data = [float(element) for element in line.split()]
        if N1 == 2 and N2 == 0:
            atom_an, _, _ = read_n_elements(3*N1*(3*N1-5), 0, line_data, file)
            atom_an = atom_an.reshape((3*N1-5, 3*N1))
            atom_an = atom_an.T
        elif N2 == 0:
