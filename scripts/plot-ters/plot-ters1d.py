...
for mode_dir in mode_dirs:
    ...
    with control_file.open() as f:
        ...
        line_height = next((l for l in lines if l.strip().startswith("tip_molecule_distance")), None)
        line_shift = next((l for l in lines if l.strip().startswith("rel_shift_from_tip")), None)
        tip_height = float(line_height.split()[-1]) if line_height else None
        tip_shift = [float(x) for x in line_shift.split()[-2:]]
...
if args.xlim:
    xlims = args.xlim
    plt.xlim(xlims)
    mask = (wns >= xlims[0]) & (wns <= xlims[1])
else:
    mask = np.ones_like(wns, dtype=bool)
...
# %%
plt.plot(w, signal, label=rf'Gaussian-broadened $\sigma={sigma}$ cm$^{{-1}}$')
plt.xlabel(r'Wavenumber [cm$^{-1}$]')
plt.ylabel(r'$\left(\mathrm{d}\alpha_{zz} / \mathrm{d}Q_\omega\right)^2$  [$e^2$ $\mathrm{\AA}^2$ $V^{-2}$]')
